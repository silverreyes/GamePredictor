...
# ---------------------------------------------------------------------------
# Test: MLFLOW_TRACKING_URI override
# ---------------------------------------------------------------------------
def test_mlflow_tracking_uri_override(monkeypatch, tmp_path):
    """Sets MLFLOW_TRACKING_URI env var, verifies mlflow.set_tracking_uri called with it."""
    from pipeline.refresh import retrain_and_stage

    monkeypatch.setenv("MLFLOW_TRACKING_URI", "http://mlflow:5000")

    engine = MagicMock()
    experiments_path = str(tmp_path / "experiments.jsonl")
    model_dir = str(tmp_path / "models")
    os.makedirs(model_dir, exist_ok=True)

    mock_df = pd.DataFrame({"game_id": ["g1"], "season": [2023], "home_win": [1]})
    mock_results = {
        "val_accuracy_2023": 0.65,
        "val_accuracy_2022": 0.60,
        "val_accuracy_2021": 0.60,
        "log_loss": 0.6,
        "brier_score": 0.22,
        "shap_top5": [("feat1", 0.1)],
    }
    ...
    with patch("pipeline.refresh.mlflow.set_tracking_uri") as mock_set_uri, \
         patch("pipeline.refresh.mlflow.set_experiment"), \
         patch("features.build.build_game_features", return_value=mock_df), \
         patch("models.train.load_and_split", return_value=(mock_df, mock_df, mock_df, mock_df, ["home_win"])), \
         patch("models.train.train_and_evaluate", return_value=(mock_results, mock_model)), \
         patch("models.baselines.compute_baselines", return_value={"always_home_accuracy": 0.5, "better_record_accuracy": 0.5}), \
         patch("models.train.should_keep", return_value=True), \
         patch("models.train.save_best_model", return_value=str(tmp_path / "models" / "best_model.json")), \
         patch("models.train.log_experiment"):
